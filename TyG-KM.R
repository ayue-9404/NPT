...
# 1️⃣ 读取数据
# ================================
df <- read_excel(file.choose())   # 选择 TyG.xlsx
print(names(df))                  # 看列名
...
df <- df %>%
  ...
  filter(!is.na(time) & !is.na(status) & !is.na(TyG))

# ================================
# 3️⃣ 中位数分组（关键🔥）
# ================================
cutoff <- median(df$TyG, na.rm = TRUE)

df$group <- ifelse(df$TyG <= cutoff, "Low TyG", "High TyG")
df$group <- factor(df$group, levels = c("Low TyG", "High TyG"))
