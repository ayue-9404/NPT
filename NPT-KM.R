...
df <- df %>%
  mutate(
    ...
    NPT    = as.numeric(`RFH-NPT score`)
  ) %>%
  filter(!is.na(time) & !is.na(status) & !is.na(NPT))

# ================================
# 3️⃣ 先三分位，再去掉中间组🔥
# ================================
df <- df %>%
  mutate(npt_tertile = ntile(NPT, 3)) %>%
  filter(npt_tertile != 2)   # ❗去掉中间组

# 二分类
df$group <- factor(df$npt_tertile,
                   levels = c(1,3),
