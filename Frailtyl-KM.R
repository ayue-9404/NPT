...
df <- df %>%
  mutate(
    ...
    FI     = as.numeric(`Frailty index`)
  ) %>%
  filter(!is.na(time) & !is.na(status) & !is.na(FI))

# ================================
# 3️⃣ 按 FI ≥ 0.25 分组（关键🔥）
# ================================
df$group <- ifelse(df$FI < 0.25, "Non-frail", "Frail")
df$group <- factor(df$group, levels = c("Non-frail", "Frail"))
